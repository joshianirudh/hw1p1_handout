...
class BatchNorm1d:
    ...
    def forward(self, Z, eval=False):
        ...
        self.M = np.sum(self.Z, axis=0) / self.N  # TODO: Calculate mini-batch mean
        self.V = np.sum((self.Z - self.M) ** 2, axis=0 )/ self.N  # TODO: Calculate mini-batch variance
    ...
    def backward(self, dLdBZ):
        ...
        dLdV = -0.5* np.sum(dLdNZ * (self.Z - self.M) * (self.V + self.eps) ** -1.5, axis=0)  # TODO: Compute gradient of loss backprop through variance calculation.
        dNZdM = - (self.V + self.eps) ** -0.5 - 0.5 * (self.Z - self.M) * (self.V + self.eps) ** -1.5 * (-2 * np.sum(self.Z - self.M, axis = 0) / self.N)  # TODO: Compute derivative of normalized input with respect to mean.
        dLdM = np.sum(dLdNZ * dNZdM, axis=0)  # TODO: Compute gradient of loss with respect to mean.

        dLdZ = dLdNZ * ( (self.V + self.eps) ** -0.5 ) + dLdV * (2/self.N * (self.Z - self.M)) + dLdM / self.N  # TODO: Compute gradient of loss with respect to the input.
